...
%          pick_pro = strfind(image_name,'???ù');
%         if pick_pro
%             continue;
%         end
%       name1 = image_name(1:10);
      ImageName = strcat(file_path,image_name);
      C(j,:) = infodcm(file_path,ImageName);
...
name = {'Path_name','PatientID','Gender','PatientName','AccessionNumber','PatientBirthDate',...
    'Age','SliceThickness','KVP','XrayTubeCurrent',...
   'PixelSpacing','Manufacturer','Exposure','AcquisitionDate'};
xlswrite('yxy_COPD_stage1.xlsx',name,'sheet1','A1');
xlswrite('yxy_COPD_stage1.xlsx',D,'sheet1','A2');
Infom = cell(5,10);
function C = infodcm(path,str)
...
s = struct('filenam',path,'PatientID',info.PatientID,'PatientName',info.PatientName.FamilyName,'AccessionNumber',info.AccessionNumber,'Gender',info.PatientSex,'PatientBirthDate',...
